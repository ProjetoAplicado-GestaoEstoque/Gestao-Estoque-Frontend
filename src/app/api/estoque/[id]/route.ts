...
export async function PUT(
  ...
  const { item_id, quantity, description, type } = await req.json();
  ...
  try {
    ...
    if (!stock) {
      return NextResponse.json({ message: "Estoque não encontrado" }, { status: 404 });
    }
    ...
    console.error(err);
    return NextResponse.json({ message: "Erro ao atualizar estoque." }, { status: 500 });
  }
}