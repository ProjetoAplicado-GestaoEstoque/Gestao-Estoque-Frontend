...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, storage, description, quantity, supplier_id, project_id } = body

    if (!name || !storage || !quantity || !supplier_id || !project_id) {
      return NextResponse.json(
        { message: 'Os campos Nome, Local de armazenamento e Quantidade devem ser preenchidos' },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json({ message: 'Produto criado com sucesso', item: newItem }, { status: 201 })
  } catch (error) {
    if (error instanceof Error) {
